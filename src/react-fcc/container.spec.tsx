...
import { TextComponent, TextComponentMappedActions, FetchingTextComponent } from './test-components/TextComponent'
...
test('initial state can be set', () => {
  ...
  const component = renderer.create(<Container foo="test" />)
...
test('state can be derived from props', () => {
  const Container = container(
    ...
    {
      derivedState: (props: { prefix: string }, state) => {
        return {
  ...
  const component = renderer.create(<Container foo="test" prefix="foo" />)
  ...
  component.update(<Container foo="test" prefix="mee" />)
...
function fetchText(updateText: (newText: string) => void) {
  setTimeout(() => {
    updateText('fetched')
  }, 100)
